...
// A simple dynamic array to hold incidents
typedef struct {
    Incident** incidents; // Array of pointers to Incident
    int size;
    int capacity;
} IncidentArray;
...
/**
 * Adds an incident to the array. O(1) complexity.
 */
void addIncidentToArray(IncidentArray* q, Incident* incident) {
...
/**
 * Finds and removes the highest priority incident. O(n) complexity.
 */
Incident* extractMaxFromArray(IncidentArray* q) {
    if (q->size == 0) {
        return NULL;
    ...
    // O(n) search for the highest priority incident
    ...
    // Remove from array by swapping with the last element (fast removal)
...
// --- Dispatcher Logic Implementation ---
...
void handleDispatch(IncidentArray* q, Unit units[], int num_units) {
    ...
    // The dispatched incident is no longer needed in memory
    free(incident_to_dispatch);
}

void handleUpdateUnitStatus(Unit units[], int num_units) {
    printf("\n--- Update Unit Status ---\n");
    // This function remains the same as the heap version
    // ... code omitted for brevity but is identical to previous C answer
    ...
    for (int i = 0; i < num_units; i++) {
        if (strcasecmp(units[i].id, unit_id_str) == 0) {
            if (units[i].status == AVAILABLE) {
                printf("\nℹ️ Unit %s is already available.\n", units[i].id);
            } else {
                units[i].status = AVAILABLE;
                units[i].assigned_incident_id = -1;
...
void handleShowStatus(IncidentArray* q, Unit units[], int num_units) {
    ...
    if (q->size == 0) {
        ...
        // Create a temporary copy to sort for display without modifying the original
    ...
    for (int i = 0; i < num_units; i++) {
        ...
        if (units[i].status == DISPATCHED) {
            printf(" to Incident ID-%d\n", units[i].assigned_incident_id);
        } else {
            printf("\n");
        }
    }
